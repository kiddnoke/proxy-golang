...
func NewConn(c net.Conn, cipher *Cipher) *SsConn {
	return &SsConn{
		Conn:     c,
		Cipher:   cipher,
...
func DialWithRawAddr(rawaddr []byte, server string, cipher *Cipher) (c *SsConn, err error) {
...
func Dial(addr, server string, cipher *Cipher) (c *SsConn, err error) {
	ra, err := RawAddr(addr)
	if err != nil {
		return
	}
...
func (c *SsConn) GetIv() (iv []byte) {
	iv = make([]byte, len(c.iv))
	copy(iv, c.iv)
	return
}

func (c *SsConn) GetKey() (key []byte) {
